...
if (-not (Test-Path (Join-Path $ProjectRoot ".venv\Scripts\python.exe"))) {
    Write-Error "Virtual environment not found. Run scripts\install-windows.ps1 first."
}
...
if (-not (Test-Path (Join-Path $ProjectRoot ".env"))) {
    Write-Warning ".env not found. Copy .env.example to .env and fill OPENAI_API_KEY."
}

$PythonPath = "$ProjectRoot;$ProjectRoot\leader;$($SdkPath.Path)"
